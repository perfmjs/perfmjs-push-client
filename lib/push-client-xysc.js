            ...
            this.getRemoteData(this.option('remoteDataUrl') + '&time=' + $$.utils.now(), function(responseText) {
                ...
                if (jsonData.status === 'success') {
                    ...
                    if ($$.utils.toNumber(jsonData.result.nextTerm) - $$.utils.toNumber(jsonData.result.openTerm) > 1) {
                        ...
                        if (($$.utils.toNumber(self.option('nextOpenTermAndTime')['nextOpenTerm']) -  $$.utils.toNumber(jsonData.result.openTerm) == 1)
                                && $$.utils.toNumber(self.option('nextOpenTermAndTime')['leftOpenTime']) > 0) {
                            nextTime = $$.utils.toNumber(self.option('nextOpenTermAndTime')['leftOpenTime']);
                        }
